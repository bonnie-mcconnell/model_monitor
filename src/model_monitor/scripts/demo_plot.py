...
def generate(output_path: Path, *, show: bool = False, seed: int = 42) -> None:
    ...
    gs = gridspec.GridSpec(
        4, 1, hspace=0.55, left=0.07, right=0.96, top=0.94, bottom=0.06
    )
    ...
    ax.axhline(
        ...
        linestyle=":",
        alpha=0.7,
        label="Warn threshold (0.75)",
    )
    ax.fill_between(
        batches,
        data["trust"],
        0.6,
        where=data["trust"] < 0.6,
        alpha=0.15,
        color="#e74c3c",
        zorder=0,
    )
    ...
    ax.fill_between(
        batches,
        0.25,
        ax.get_ylim()[1] if ax.get_ylim()[1] > 0.25 else 0.6,
        alpha=0.08,
        color="#e74c3c",
    )
    ...
    for psi_key, color, label in zip(
        ...
        ax.plot(
            batches, data[psi_key], color=color, linewidth=1.4, label=label, alpha=0.9
        )
    ...
    ax.legend(
        fontsize=7, loc="upper left", facecolor=ACCENT, labelcolor=TEXT, framealpha=0.8
    )

    # ── Panel 3: Output drift + conformal set size ─────────────────────────
    ax = axes[2]
    ax.set_title(
        "Output Drift (PSI)  ·  Conformal Prediction Set Size", fontsize=10, pad=4
    )
    ax.set_ylabel("Output PSI", fontsize=8, color="#e67e22")
    ax.tick_params(axis="y", colors="#e67e22")
    ax.plot(
        batches, data["output_psi"], color="#e67e22", linewidth=1.6, label="Output PSI"
    )
    ...
    lines = [
        ...
        Line2D(
            [0], [0], color="#2ecc71", linewidth=1.5, linestyle="-.", label="Set size"
        ),
    ]
    ax.legend(
        handles=lines,
    ...
    ax.set_ylim(bottom=0)
    ax.legend(
        fontsize=7, loc="upper right", facecolor=ACCENT, labelcolor=TEXT, framealpha=0.8
    )
